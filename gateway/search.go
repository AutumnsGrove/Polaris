...
// handleSearch backs Atlas's results page — a thin HTTP wrapper around
// ResolveAtlasSearch (gateway/atlas_search.go), which does the actual
// SearXNG/Brave/cache/virtual-paging work, shared with the CLI's
// bare-metal `polaris atlas search` path so both exercise identical
// logic. Domain ranking and the blocklist apply identically whether a
// query comes from a person typing into Atlas or the assistant calling
// web_search on their behalf, since both share the same SearXNGClient.
func (s *Server) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	resp, _, err := ResolveAtlasSearch(r.Context(), s.db, s.searxng, s.brave, query, category, page, maxResults)
	if err != nil {
