...
func handleWebRead(argsJSON string, ctx *Context) string {
	...
	title, siteName, imageURL, text, err := fetchAndExtract(ctx.Ctx, args.URL, ctx.Blocklist)
	...
	if err != nil || looksLikePaywall(text) {
		if wbTitle, wbSiteName, wbImageURL, wbText, wbErr := fetchFromWayback(ctx.Ctx, args.URL, ctx.Blocklist); wbErr == nil {
			title, siteName, imageURL, text, err = wbTitle, wbSiteName, wbImageURL, wbText, nil
...
func fetchAndExtract(ctx context.Context, rawURL string, blocklist *search.Blocklist) (title, siteName, imageURL, text string, err error) {
	client := &http.Client{
	...
	return title, siteName, imageURL, text, nil
}
...
func fetchFromWayback(ctx context.Context, rawURL string, blocklist *search.Blocklist) (title, siteName, imageURL, text string, err error) {
